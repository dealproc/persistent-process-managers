...
#nullable enable
    public DataStore(string name, string? persistentStorePath = null) {
        ...
        GetOrCreateStream("$streams");

        _inboundEventBus = new InMemoryBus(nameof(_inboundEventBus), false);
        // We no longer subscribe projection handlers to the bus as they'll be called directly
        // from AppendToStream. We keep the bus for other subscribers like Subscription instances.
...
#nullable disable
    ...
    public static DataStore OnDisk(string name, string directory, string file = "DataStore") =>
        new DataStore(name, Path.Combine(directory, file));
    ...
    public WriteResult AppendToStream(
        ...
        if (stream.StartsWith("$"))
            throw new ArgumentException($"Stream {stream} cannot start with $, it is reserved for system projections", nameof(stream));
        ...
        for (var i = 0; i < events.Length; i++) {
            ...
            // Create the event written message
            var eventWritten = new EventWritten(stream, recordedEvent, false, recordedEvent.EventNumber);

            // Process projections synchronously instead of queuing them
            WriteToByCategoryProjection(eventWritten);
            WriteToByEventProjection(eventWritten);

            // Still notify subscribers about the event via the queue
            _inboundEventHandler.Handle(eventWritten);
        }
    ...
    private IStream GetOrCreateEventTypeStream(EventWritten @event, out string streamName) {
        ...
        // Ensure the stream is created and registered in the $streams list
        return GetOrCreateStream(streamName);
    }
    ...
    private IStream GetOrCreateStream(string streamName) {
        ...
        lock (_store) {
            if (!_store.TryGetValue(streamName, out stream)) {
                ...
                var streams = _store["$streams"];
                // Create a reference to the stream in $streams (if not already there)
                var streamReference = new RecordedEvent("$streams", Guid.NewGuid(), streams?.Count ?? 0, "$@", Helper.UTF8NoBom.GetBytes(streamName), Array.Empty<byte>(), false, DateTime.UtcNow, 0);
                streams.Add(streamReference);
                lock (_allStream) {
                    _allStream.Add(streamReference);
                }
